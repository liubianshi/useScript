...
selbgcolor='#dd6003'
selfgcolor='#e9e9e9'
bmarks=~/.config/surf/bookmarks.txt
xid=$1
p=$2
uri=`xprop -id $xid _SURF_URI | cut -d '"' -f 2`
kw=`xprop -id $xid _SURF_FIND | cut -d '"' -f 2`
dmenu="dmenu -nb $normbgcolor -nf $normfgcolor -sb $selbgcolor -sf $selfgcolor"
s_xprop() {
...
"_SURF_BMARK")
	...
	;;
"_SURF_URI_RAW")
    sel=`buku -p -f 4 | sort -nrk1 | fmenu | cut -f 1-2`
	[ -z "$sel" ] && exit
...
"_SURF_URI")
	...
	"y") # youtube
		...
		;;
    "q")
        sh -c "nohup qutebrowser \"$uri\" >/dev/null 2>&1" & disown
        exit
        ;;
	*)
		ret="$sel"
